...
2. **HTTP-only.** No WebSocket anywhere. Empirically dead (DO free-tier, reconnect storms, hybrid complexity). Extension uses `fetch()` for everything.
3. **Macro-chunked.** Each operation is a standalone JSON macro file. Signup, email-verify, create-workspace, create-webhook, activate-trial — each is its own file. Verified individually, then composed into flows.
4. **Email verification is a first-class reusable chunk** — not embedded in a signup macro. Takes configurable inputs: email worker URL, token, address, regex, subject filter, poll interval, timeout. Other macros call it via a "playlist" or reference its outputs.
5. **Turso direct for MVP.** Extension writes per-step results + captured tokens directly to Turso (libSQL HTTP API). No backend, no CF Worker, no Python daemon. Generous free tier, survives container recycles.
6. **CF Worker dashboard comes later** (Phase 2). Reads from the same Turso DB. Provides CRUD UX + can trigger macros by writing to a `command_queue` table the extension polls. No WS — pure HTTP via Turso.
7. **Per-service preset macros.** When a chunk is verified end-to-end against a live service, it gets saved under `extension/macros/<service>/<chunk>.json`. The popup shows a per-service preset picker.
8. **Detailed logs always on.** Every step logs structured events. Extension keeps a ring buffer (500 entries). Optionally pushes to Turso for permanent record. The popup renders a per-step checklist + raw log.
...
│   - workerUrl (OPTIONAL — empty = no command source)              │
└────────────────────────────────────────────────────────────────────┘
              │                              │
              │ (optional, HTTP)              │ (optional, HTTP)
              ▼                              ▼
   ┌──────────────────────┐      ┌──────────────────────────────┐
   │   Turso (libSQL)    │      │  CF Worker (Phase 2, later)  │
   │  - macro_runs       │      │  - dashboard HTML            │
   │  - step_results     │      │  - CRUD on Turso tables      │
   │  - accounts         │      │  - trigger macros (write to  │
   │  - workspaces       │      │    command_queue in Turso)    │
   │  - command_queue    │      │  - NO WS, NO DO              │
   │    (Phase 2 only)   │      └──────────────────────────────┘
   └──────────────────────┘
```
...
**Step 1: Fork and strip the notion v0.7 extension** (~30 min)
...
- Strip all WS code from `background.js` — keep only the macro runner + HTTP fetch handlers + popup message handlers
- Strip the WS connection logic entirely (the `connect()` / `ws.onopen` / `ws.onmessage` / HTTP polling fallback — all gone)
- The extension becomes a pure macro runner with HTTP fetch capability
...
- The WS reconnection logic + HTTP polling fallback (the source of 5+ P0 bugs)
- The Python daemon (can't stay running from the sandbox)
...
onboard-automation-common/
  extension-v2/                          ← the actual extension (Phase 1)
    manifest.json                        ← "Onboard Automation Bridge", no WS perms
    background.js                        ← fork from notion v0.7, WS stripped
  ...
  worker-v2/                              ← Phase 2 (CF Worker dashboard)
    src/
...
2. **Repo structure** — keep the existing `onboard-automation-common/` repo, create a `v2/` subdir, or new repo? My recommendation: keep the repo, put the new extension under `extension-v2/`, move the old untested chassis to `docs/v1-design/`.
...
5. **Should we start Phase 1 in the next session, or do you want to finish notion v0.7 stabilization first?** The plan forks notion v0.7's extension AS-IS at commit `129dc11` — if there are more fixes coming, we fork after those land.
